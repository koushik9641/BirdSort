...
namespace NiobiumStudios
{
    ...
    public class DailyRewardsInterface : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            string lastShownTimeString = PlayerPrefs.GetString("lastShownTime", DateTime.MinValue.ToString());
            lastShownTime = DateTime.Parse(lastShownTimeString);

            if (DateTime.Now > lastShownTime.AddDays(1))
            {
                ...
                // Set the lastShownTime variable to the current date and time
                lastShownTime = DateTime.Now;

                // Save this information using PlayerPrefs
                PlayerPrefs.SetString("lastShownTime", lastShownTime.ToString());
                PlayerPrefs.Save();
            }
        }
