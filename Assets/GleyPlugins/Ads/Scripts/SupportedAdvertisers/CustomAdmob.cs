    ...
    public class CustomAdmob : MonoBehaviour, ICustomAds
    {
        ...
        private void BannerLoadFailed(object sender, AdFailedToLoadEventArgs e)
        {
            if (debug)
            {
                ...
                Debug.Log("Admob Banner Failed To Load " + e.LoadAdError.GetMessage());
                ScreenWriter.Write("Admob Banner Failed To Load " + e.LoadAdError.GetMessage());
            }
            if (banner != null)
            {
                banner.OnAdLoaded -= BannerLoadSucces;
        ...
        private void InterstitialFailed(object sender, AdFailedToLoadEventArgs e)
        {
            if (debug)
            {
                ...
                Debug.Log("Admob Interstitial Failed To Load " + e.LoadAdError.GetMessage());
                ScreenWriter.Write("Admob Interstitial Failed To Load " + e.LoadAdError.GetMessage());
            }
        ...
        private void RewardedVideoFailed(object sender, AdFailedToLoadEventArgs args)
        {
            if (debug)
            {
                Debug.Log("Admob Rewarded Video Failed " + args.LoadAdError.GetMessage());
                ScreenWriter.Write("Admob Rewarded Video Failed " + args.LoadAdError.GetMessage());
