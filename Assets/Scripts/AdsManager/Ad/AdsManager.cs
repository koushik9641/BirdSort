...
public class AdsManager : SingletonDontDestroy<AdsManager>
{
    ...
    [SerializeField] private bool _showBannerOnLoad = false;
    ...
    {
        Debug.Log("LoadInterstitial " + _isAlreadyInsterstitialLoaded);
    ...
    public bool IsRewardLoaded()
    {
        bool val = false;
        if (_currentRewardNetwork == AdNetwork.AdMob)
            val = _adMobManager.RewardedAd.CanShowAd();
        else
    ...
    public void RewardEarned()
    {
        ...
        Debug.Log("AdsManager Reward Earned: " + _currentRewardNetwork);

        Debug.Log(_rewardCallback == null) ;
        Debug.Log(_rewardCallback.rewardEarned == null);
