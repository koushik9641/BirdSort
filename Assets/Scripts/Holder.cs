...
public class Holder : MonoBehaviour
{
    ...
    [SerializeField] private Sprite Closedbottle;
    [SerializeField] private Sprite Openbottle;
    ...
    //public bool bottleolosed=false;
    ...
    public Liquid RemoveTopBall()
    {
        if (_liquids.Count == 0)
            throw new InvalidOperationException();

        var liquid = _liquids.Last();
    ...
    public void Move(Liquid liquid)
    {
        if (IsPending)
        {
            IsPending = false;
    ...
    public IEnumerator UndoMoveAndTransferLiquid(Holder holder ,float transferliquid)
    {
        IsPending = false;
        ...
        //print(targetHolderPoint);
        //print(targetHolderRotation);

        var thisLiquid = _liquids.Last();

        //print("thisLiquid:"+thisLiquid);
        ...
        var transferValue = transferliquid;

        //print("thisLiquidStartValue:"+thisLiquidStartValue);
        //print("transferValue:"+transferValue);
        ...
        //print("targetLiquid:"+targetLiquid.GroupId);
        //print("targetLiquidStartValue:"+targetLiquidStartValue);

        if (targetLiquid.GroupId != thisLiquid.GroupId)
        {
            holder.AddLiquid(thisLiquid.GroupId);
            targetLiquid = holder.Liquids.Last();
            targetLiquidStartValue = targetLiquid.Value;
        }

        //var transferliquidtop=holder.Liquids.Last();
...
/*        if(AudioManager.IsSoundEnable){
        ...
        // If bottle is full chnage the Sprite

        //print(holder.Liquids.ToList().Count);
        //print(holder.CurrentTotal);
    ...
    public IEnumerator MoveAndTransferLiquid(Holder holder,Action onLiquidTransferComplete=null )
    {
        ...
        //print(holder.transform.position);
        //print(deliverTopPosition);
        //print(holder);
...
/*        if(AudioManager.IsSoundEnable){
        ...
        if((holder.Liquids.ToList().Count == 1) && (holder.CurrentTotal == MAXValue )){

           // print("Bottle Full");
           holder.transform.GetChild(1).GetComponent<SpriteRenderer>().sprite=Closedbottle;
           //bottleolosed=true;
    ...
    {
        return transform.GetChild(0).GetChild(Mathf.FloorToInt(CurrentTotal)-1).position;
    ...
    public void Init(IEnumerable<LiquidData> liquidDatas,bool isliquidvalid)
    {
        if(isliquidvalid){
